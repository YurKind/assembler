...
start:
    ...
    mov ah, byte ptr[symbol]            ; копируем искомый символ
    mov edi, offset buf                 ; копируем адрес начала буфера

find_symbol:
    mov al, byte ptr[esi]       ; помещаем в регистр al символ на который указывает esi
    ...
    mov byte ptr[edi], al       ; иначе формируем часть строки (символы предшествующие введённому символу)
...
string_overwriting:             ; перезапись исходной строки
    ...
    shaping:                    ; формируем строку
        mov al, byte ptr[edi]   ; в al помещаем первый символ из буфера
        cmp al, 0               ; сравниваем с концом строки
        je done                 ; если конец, то переходим к выводу сообщения, о том, что строка сформирована
        mov byte ptr[esi], al   ; иначе перемещаем в исходную строку символ из буфера
        inc edi                 ; 'перемещаем' edi
        inc esi                 ; 'перемещаем' esi
